...
embedding_model_dict_list = ["text2vec", "openai"]

llm_model_dict_list = ["chatglm-6b", "openai"]

from bot import LocalDocQA
...
<b>Title：</b>{doc.page_content}
</details>""" for i, doc in enumerate(resp["source_documents"])])
        # history[-1][-1] += source
        history = history + [[query, source]]
    else:
...
def reinit_model(
    llm_model, embedding_model, llm_history_len, use_ptuning_v2, top_k, history
):
    try:
        local_doc_qa.init_cfg(
            llm_model=llm_model,
...
def add_vs_name(vs_name, vs_list, chatbot):
    if vs_name in vs_list:
        ...
        return (
            gr.update(visible=True, choices=vs_list + [vs_name], value=vs_name),
            vs_list + [vs_name],
            chatbot,
        )
...
with gr.Blocks(css=block_css) as demo:
    vs_path, file_status, model_status, vs_list = (
        gr.State(""),
        gr.State(""),
        ...
        with gr.Row():
            ...
            with gr.Column(scale=5):
                ...
                with vs_setting:
                    ...
                    vs_name = gr.Textbox(label="请输入新建知识库名称", lines=1, interactive=True)
    ...
    with gr.Tab("模型配置"):
        llm_model = gr.Radio(
            llm_model_dict_list, label="LLM 模型", value=LLM_MODEL, interactive=True
        )
        llm_history_len = gr.Slider(
            0, 10, value=LLM_HISTORY_LEN, step=1, label="LLM 对话轮数", interactive=True
        )
        use_ptuning_v2 = gr.Checkbox(
            False, label="使用p-tuning-v2微调过的模型", interactive=True
        )
